...
public interface EventsNextPageApi {
    ...
    ResponseEntity<PageEvents> nextEvents(@ApiParam(value = "a 'cursor' for retreiving the next page of Events.", required = true) @PathVariable("nextToken") String nextToken, @Min(0) @Max(1000) @ApiParam(value = "Maximum number of records to return.", allowableValues = "") @Valid @RequestParam(value = "limit", required = false) Integer limit);
